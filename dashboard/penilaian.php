...
<div class="card shadow mb-4">
    ...
    <form action="" method="GET">
        <input type="hidden" name="halaman" value="penilaian">
        <div class="card-body">
            <div class="row">
                <div class="col-sm-4">
...
<div class="card shadow mb-4">
    <div class="card-header py-3 d-flex align-items-center justify-content-between">
        <h6 class="m-0 font-weight-bold">Data Penilaian</h6>
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table id="dataTable" class="table table-bordered">
                ...
                <tbody>
                    <?php $nomor = 1; ?>
                    <?php $ambil = $koneksi->query("SELECT * FROM pelamar JOIN status ON pelamar.id_status = status.id_status WHERE pelamar.id_lowongan='$id_lowongan' AND pelamar.id_status IN (2,4,5,6,7,8,9)"); ?>
                    <?php while ($pecah = $ambil->fetch_assoc()) { ?>
                        <tr>
                            ...
                            <td colspan="2">
                                <?php
                                $query = mysqli_query($koneksi, "SELECT*FROM penilaian WHERE id_pelamar='$pecah[id_pelamar]'");
                                $check = mysqli_num_rows($query);
                                $data = mysqli_fetch_assoc($query);
                                if ($check > 0) {
                                ?>
                                    <a href="index.php?halaman=ubah-penilaian&id=<?php echo $data['id_penilaian']; ?>" class="btn btn-info" title="">Ubah</a>
                                <?php
                                } else {
                                ?>
                                    <a href="index.php?halaman=tambah-penilaian&id=<?php echo $pecah['id_pelamar']; ?>" class="btn btn-info" title="">Tambah</a>
                                <?php
                                }
                                ?>
                            </td>
